...
def _status_from_times(now: datetime, start_time: Optional[datetime], end_time: Optional[datetime]) -> str:
...
def build_competition_payload(row, now: Optional[datetime] = None) -> CompetitionStatusPayload:
    ...
    start_time = _parse_iso_datetime(row["start_time"], "start_time")
    end_time = _parse_iso_datetime(row["end_time"], "end_time")
    manual_status = row["manual_status"]
    effective_status = manual_status if manual_status in VALID_MANUAL_STATUSES else _status_from_times(
        current_time,
        start_time,
        end_time,
    )
    ...
    if effective_status == "not_started" and start_time:
        seconds_until_start = max(int((start_time - current_time).total_seconds()), 0)
...
def _validate_single_running_competition(
    ...
    *,
    ...
    candidate_row = {
        ...
        "competition_name": "",
        "start_time": next_start_time.isoformat() if next_start_time else None,
...
def create_competition(competition_name: str) -> CompetitionPayload:
    ...
    with get_connection() as connection:
        ...
        connection.execute(
            ...
            """,
            (competition_id, normalized_name, now, now),
        )
        connection.commit()
...
def update_competition_settings(
    *,
    ...
    parsed_start = _parse_iso_datetime(start_time, "start_time")
    ...
    with get_connection() as connection:
        if manual_status == "running":
            ...
            _validate_single_running_competition(
                ...
                next_start_time=_parse_iso_datetime(current_row["start_time"], "start_time"),
